...
class UserControllerTest < ActionDispatch::IntegrationTest
  ...
  test 'USER-S002' do
    ...
    post login_url params: { email: 'email@email.jp', password: 'password' }, headers: { Authorization: token },
                   as: :json
  ...
  test 'USER-S003' do
    ...
    # emailが存在しない
    post login_url params: { email: 'emailio@email.jp', password: 'password' }, headers: { Authorization: token },
                   as: :json
    login_res = JSON.parse(response.body)
    assert_not_nil(login_res['error'])
  end
